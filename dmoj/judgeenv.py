...
log_file = (
    server_host
) = server_port = no_ansi = no_watchdog = problem_regex = case_regex = None
...
def load_env(cli=False, testsuite=False):  # pragma: no cover
    global problem_dirs, only_executors, exclude_executors, log_file, server_host, server_port, no_ansi, no_ansi_emu, env, startup_warnings, no_watchdog, problem_regex, case_regex, api_listen, secure, no_cert_check, cert_store, problem_watches, cli_command

    if cli:
        description = (
            'Starts a shell for interfacing with a local judge instance.'
        )
    else:
    ...
    if not cli:
        parser.add_argument(
            'server_host', help='host to connect for the server'
        )
        parser.add_argument(
        ...
        parser.add_argument(
            'command',
            nargs='*',
            help='invoke CLI command without spawning shell',
        )
    ...
    if testsuite:
        parser.add_argument(
            'tests_dir', help='directory where tests are stored'
        )
        parser.add_argument(
            'problem_regex',
            help='when specified, only matched problems will be tested',
            nargs='?',
        )
        parser.add_argument(
            'case_regex',
            help='when specified, only matched cases will be tested',
            nargs='?',
        )
    ...
    only_executors |= (
        args.only_executors and set(args.only_executors.split(',')) or set()
    )
    exclude_executors |= (
        args.exclude_executors
        and set(args.exclude_executors.split(','))
        or set()
    )
    ...
    with open(model_file) as init_file:
        ...
        if problem_dirs is None:
            if not testsuite:
                raise SystemExit(
                    'problem_storage_root not specified in "%s"; '
        ...
        for dir in problem_dirs:
            if isinstance(dir, ConfigNode):
                for depth, recursive_root in dir.iteritems():
                    try:
                        dirs += find_directories_by_depth(
                            get_path(_root, recursive_root), int(depth)
                        )
                    except ValueError:
                        startup_warnings.append(
                            'illegal depth argument %s' % depth
                        )
            else:
    ...
    if warnings:
        ...
        for dir in dirs:
            if not os.path.exists(dir) or not os.path.isdir(dir):
                startup_warnings.append(
                    'cannot access problem directory %s (does it exist?)' % dir
                )
                continue
    ...
    for dir in get_problem_roots():
        for problem in os.listdir(dir):
            ...
            if os.access(os.path.join(dir, problem, 'init.yml'), os.R_OK):
                problems.append(
                    (problem, os.path.getmtime(os.path.join(dir, problem)))
                )
    return problems
