...
class IsolateTracer(dict):
    ...
    def check_file_access_at(self, syscall, is_open=False):
        def check(debugger):
            ...
            file, accessible = self._file_access_check(
                file, debugger, is_open, dirfd=debugger.arg0, flag_reg=2
            )
            if accessible:
                return True
    ...
    def _get_fs_jail(self, debugger, is_write):
        ...
        if fs is None:
            ...
            if fs_parts:
                fs_re = '|'.join(
                    map(lambda p: p.format(pid=debugger.pid), fs_parts)
                )
            else:
    ...
    def _file_access_check(
        ...
        try:
            ...
            log.exception(
                'Unicode decoding error while opening relative to %d: %r',
                dirfd,
                rel_file,
            )
            return '(undecodable)', False

        is_write = is_open and self.is_write_flags(
            getattr(debugger, 'uarg%d' % flag_reg)
        )
    ...
    def get_full_path(self, debugger, file, dirfd=AT_FDCWD):
        ...
        if not file.startswith('/'):
            dir = (
                self._getcwd_pid(debugger.pid)
                if dirfd == AT_FDCWD
                else self._getfd_pid(debugger.pid, dirfd)
            )
    ...
    def do_kill(self, debugger):
        ...
        return (
            True if debugger.uarg0 == debugger.pid else ACCESS_EPERM(debugger)
        )

    def do_prlimit(self, debugger):
        return (
            True
            if debugger.uarg0 in (0, debugger.pid)
            else ACCESS_EPERM(debugger)
        )
