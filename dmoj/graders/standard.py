...
class StandardGrader(BaseGrader):
    def grade(self, case):
        ...
        result.extended_feedback = (
            check.extended_feedback or result.extended_feedback
        )
    ...
    def check_result(self, case, result):
        ...
        if not result.result_flag or getattr(
            checker.func, 'run_on_error', False
        ):
            try:
                check = checker(
                    result.proc_output,
    ...
    def _interact_with_process(self, case, result, input):
        ...
        try:
            result.proc_output, error = process.communicate(
                input,
                outlimit=case.config.output_limit_length,
                errlimit=1048576,
            )
