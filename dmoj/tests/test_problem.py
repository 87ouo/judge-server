...
class ProblemTest(unittest.TestCase):
    ...
    def test_test_case_matching(self):
        ...
        self.assertEqual(
            MockProblem('test', 2, 16384, {}).config.test_cases.unwrap(),
            [
                {
                    'batched': [
                        {'in': 's2.1-1.in', 'out': 's2.1-1.out'},
                        {'in': 's2.1.2.in', 'out': 's2.1.2.out'},
                    ],
                    'points': 1,
                },
    ...
    def test_bad_init(self):
        ...
        with self.assertRaisesRegex(
            InvalidInitException, 'while scanning a quoted scalar'
        ):
            Problem('test', 2, 16384, {})
    ...
    @unittest.skipIf(
        os.devnull != '/dev/null', 'os.path.exists("nul") is False on Windows'
    )
