...
class Judge:
    ...
    def _block_and_grade(
        self, problem, language, source, short_circuit, report=print
    ):
        ...
        try:
            self.current_grader = grader_class(
                self, problem, language, utf8bytes(source)
            )
        ...
        self.packet_manager.begin_grading_packet(
            self.current_grader.is_pretested
        )
        ...
        try:
            for result in self.grade_cases(
                ...
                if isinstance(result, BatchBegin):
                    ...
                    report(
                        ansi_style(
                            "#ansi[Batch #%d](yellow|bold)" % batch_counter
                        )
                    )
                    in_batch = True
                    ...
                    colored_aux_codes = (
                        '{%s}' % ', '.join(colored_codes[1:])
                        if len(codes) > 1
                        else ''
                    )
                    colored_feedback = (
                        '(#ansi[%s](|underline)) ' % utf8text(result.feedback)
                        if result.feedback
                        else ''
                    )
                    case_info = (
    ...
    def begin_grading(
        ...
        def grading_cleanup_wrapper():
            ...
            try:
                ...
                self._block_and_grade(
                    problem, language, source, short_circuit, report=report
                )
            except Exception:
        ...
        self.current_submission_thread = threading.Thread(
            target=grading_cleanup_wrapper
        )
    ...
    def grade_cases(
        self, grader, cases, short_circuit=False, is_short_circuiting=False
    ):
        for case in cases:
            ...
            if isinstance(case, BatchedTestCase):
                ...
                for batched_case in self.grade_cases(
                    ...
                    if (batched_case.result_flag & Result.WA) and (
                        not case.points or short_circuit
                    ):
            ...
            if (result.result_flag & Result.WA) > 0 and (
                short_circuit or not case.points
            ):
                is_short_circuiting = True
...
class ClassicJudge(Judge):
    def __init__(self, host, port, **kwargs):
        super().__init__()
        self.packet_manager = packet.PacketManager(
            host, port, self, env['id'], env['key'], **kwargs
        )
...
def sanity_check():
    if os.name == 'nt':
        ...
        try:
            from .cptbox import (
                _cptbox,
            )  # noqa: F401, we want to see if this imports
        except ImportError:
...
class JudgeManager:
        ...
        while self._try_respawn or self.pids:
            ...
            if pid in self.pids:
                ...
                if self._try_respawn:
                    logpm.warning(
                        'Judge died, respawning: %s (pid %d, 0x%08X)',
                        judge,
                        pid,
                        status,
                    )
                    ...
                    logpm.info(
                        'Judge exited: %s (pid %d, 0x%08X)', judge, pid, status
                    )
                ...
                if self._try_respawn:
                    logpm.warning(
                        'API server died, respawning (0x%08X)', status
                    )
                    self._spawn_api()
                else:
    ...
    def _respawn_judges(self, signum, frame):
        logpm.info(
            'Received signal (%s), murderizing all children.',
            self.signal_map.get(signum, signum),
        )
    ...
    def run(self):
        ...
        setproctitle(
            'DMOJ Judge: Process manager on %s' % (make_host_port(judgeenv),)
        )
