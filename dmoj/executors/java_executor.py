...
reclass = re.compile(
    r'\bpublic\s+(?:strictfp\s+)?(?:(?:abstract|final)\s+)?(?:strictfp\s+)?class\s+([\w\$][\w\$]*?)\b',
    re.U,
)
repackage = re.compile(r'\bpackage\s+([^.;]+(?:\.[^.;]+)*?);', re.U)
reexception = re.compile(
    r'7257b50d-e37a-4664-b1a5-b1340b4206c0: (.*?)$', re.U | re.M
)

JAVA_SANDBOX = os.path.abspath(
    os.path.join(os.path.dirname(__file__), 'java_sandbox.jar')
)

with open(
    os.path.join(os.path.dirname(__file__), 'java-security.policy'), 'r'
) as policy_file:
...
def find_class(source):
    ...
    if package:
        raise CompileError(
            'Invalid package %s: do not declare package\n' % package.group(1)
        )
    return class_name
...
class JavaExecutor(CompiledExecutor):
    ...
    def get_cmdline(self):
        agent_flags = '-javaagent:%s=policy:%s' % (
            self._agent_file,
            self._policy_file,
        )
    ...
    def parse_feedback_from_stderr(self, stderr, process):
        if process.returncode:
            try:
                with open(
                    os.path.join(self._dir, 'submission_jvm_crash.log'), 'r'
                ) as err:
                    log = err.read()
    ...
    def initialize(cls):
        if cls.get_vm() is None or cls.get_compiler() is None:
            return False
        if not os.path.isfile(cls.get_vm()) or not os.path.isfile(
            cls.get_compiler()
        ):
            return False
