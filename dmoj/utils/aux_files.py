...
def compile_with_auxiliary_files(filenames, flags, lang=None, compiler_time_limit=None, cached=True):
    ...
    if use_cpp or use_c:
        ...
        executor = executor('_aux_file', None, aux_sources=sources, cached=cached)
    else:
        if len(sources) > 1:
